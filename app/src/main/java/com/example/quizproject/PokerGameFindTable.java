...
public class PokerGameFindTable extends BaseActivity implements View.OnClickListener {
            ...
            dataBaseRef.child("pokerTableGames").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {//לקחנו את כל המסמך
                ...
                public void onComplete(@NonNull Task<DataSnapshot> task) {
                    if (!task.isSuccessful()) {
                        ...
                        HashMap<String,Object> resultPokerTableGames = (HashMap<String, Object>) task.getResult().getValue();
                        if (task.getResult().getValue()!=null) {
                            addTableGame((HashMap<String, Object>) resultPokerTableGames.get("pokerTables"), (HashMap<String, Object>) resultPokerTableGames.get("usersToTables"),(HashMap<String, Object>) resultPokerTableGames.get("activeGames"));//הבאנו פעם אחת את הטבלה של השולחנות ופעם של משתמשים
                            downloadFireStoreInBackground();
                        }
                        else{
                            addTableGame(null,null,null);
                            downloadFireStoreInBackground();
                        }
                        //       }
                    }
    ...
    public void addTableGame(HashMap<String,Object> resultPokerTableGames,HashMap<String,Object> resultPokerUsersToTables, HashMap<String, Object> resultPokerTableGamesActive) {
        int num = 1000 + (int) (Math.random() * 1000);//1000-2000
        if (resultPokerTableGames != null) {
            boolean isKeyPresent = resultPokerTableGames.containsKey("" + num) || resultPokerTableGamesActive.containsKey("" + num) ;
            if (isKeyPresent) {
                addTableGame(resultPokerTableGames, resultPokerUsersToTables,resultPokerTableGamesActive);
                return;
            } else {
