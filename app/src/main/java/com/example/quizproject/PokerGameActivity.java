...
public class PokerGameActivity extends AppCompatActivity implements View.OnClickListener, GameClock.ClockListener {
    ...
    //מגדיר את ה GAME CLOCK של המשחק בכל תור
    GameClock gameClock;
    ...
    ValueEventListener listenerEndAllGame;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        musicServiceIntent=new Intent(this, BackgroundMusic.class );
...
////////before start game
                    ...
                    try {
                        ...
                        for(Map.Entry<String,Integer> entry : usersDetails.entrySet()) {
                            if(entry.getKey().replace("*",".").equals(mAuth.getCurrentUser().getEmail())){
                                myPlace=i;
                                yourMoney=Integer.parseInt(""+entry.getValue());
                            }
                            emailsInGame.add(entry.getKey().replace(".","*"));
                            i++;
                        }
                        openTableUserDetails(emailsInGame);
    ...
    public void openTableUserDetails(ArrayList<String> emailsInGame){
...
//4485454
                    ...
                    try {
                        ...
                        for(Map.Entry<String,String> Entry : resultPokerTableGames.entrySet()) {
                            if (Entry.getValue().contains("fold")){
                                ...
                                if (!Entry.getValue().contains("wait")) {
                                    arrayList.add(Integer.parseInt(Entry.getValue().split(":")[1]));
                                }
                            }
                            if(Entry.getValue().contains("allIn")){
                                checkAllIns++;
                        ...
                        else if (checkAllIns==usersArrListInGame.size()-1){//בגלל ששיניתי לשני שחקנים אז:
                        ...
                        for(Map.Entry<String,String> entry : resultPokerTableGames.entrySet()) {
                            if (entry.getValue().equals("wait")){
                                ...
                                arrayList1.add(Integer.parseInt(entry.getValue().split(":")[1]));
                            }
                            if (entry.getValue().contains("raise")){//אם שחקן מסויים העלה אז לקחת את ההעלעה הגבוהה ביותר
                        ...
                        //עשינו שיש רק 2 שחקנים
                        if (arrayList1.size()==2 && arrayList1.get(0).equals(arrayList1.get(1) )){
                            everyOnePlayed=true;
                        }
                        if (everyOnePlayed){
                            for(Map.Entry<String,String> entry : resultPokerTableGames.entrySet()) {
                    ...
                    dataBaseRef.child("pokerTableGames").child("activeGames").child("" + tableNum).child("tableDetails").child("endGame").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            ...
                            if (numGames==numGamesPlayedCounter) {
                                ...
                                CountDownTimer temp = new CountDownTimer(5000, 5000) {
                                    @Override
    ...
    public void addTimerToGame(){
        if (gameClock!=null) {
            ...
            gameClock = new GameClock(this, 30);
            gameClock.run();
    ...
    protected void onStop() {
        ...
        // stop the background music service when the activity is stopped
    ...
    protected void onRestart() {
        ...
        // start the background music service when the activity is restarted
        startService(musicServiceIntent);
    }

    @Override
    public void onSecondTickClock(int seconds) {
        textViewTimer.setText("00"+":"+seconds);

    }

    @Override
    public void onFinishClock() {
        if (callbtnicon.isClickable()) {
            timerIsDoneMyTurn();
        }
    }
/*
